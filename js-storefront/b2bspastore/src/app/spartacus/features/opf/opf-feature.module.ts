...
@NgModule({
  ...
  providers: [
    provideConfig(<CmsConfig>{
      featureModules: {
        [OPF_QUICK_BUY_FEATURE]: {
          module: () => import('@spartacus/opf/quick-buy').then((m) => m.OpfQuickBuyModule),
        },
      },
    }),
    provideConfig(<CmsConfig>{
    ...
    provideConfig(<CmsConfig>{
      featureModules: {
        [OPF_CTA_FEATURE]: {
          module: () => import('@spartacus/opf/cta').then((m) => m.OpfCtaModule),
        },
      },
    }),
    provideConfig(<CmsConfig>{
      featureModules: {
        [OPF_BASE_FEATURE]: {
          module: () => import('@spartacus/opf/base').then((m) => m.OpfBaseModule),
        },
      },
    }),
    ...
    provideConfig(<CmsConfig>{
      featureModules: {
        [OPF_PAYMENT_FEATURE]: {
          module: () => import('@spartacus/opf/payment').then((m) => m.OpfPaymentModule),
        },
      },
    }),
    ...
    provideConfig(<CmsConfig>{
      featureModules: {
        [OPF_CHECKOUT_FEATURE]: {
          module: () => import('@spartacus/opf/checkout').then((m) => m.OpfCheckoutModule),
        },
      },
    }),
    provideConfig(<I18nConfig>{
